...
var GPlus = function () {
	...
	_createClass(GPlus, [{
		...
		key: 'calculateNewCardTitles',
		value: function calculateNewCardTitles(backendCollection, scrapedCollection) {
			console.log('\nCalculating new card titles ...');

			var backendNames = this.getNames(backendCollection),
			    scrapedNames = this.getNames(scrapedCollection);

			return scrapedNames.filter(function (name) {
				return !backendNames.find(function (name) {
					return name;
				});
			});
		}
		...
		key: 'updateBackend',
		value: function updateBackend(scrapedCollection, req, res) {
			var _this2 = this;
			...
			backendPromise.then(function (backendCollection) {
				var newCardNames = _this2.calculateNewCardTitles(backendCollection, scrapedCollection);

				var addtoPromise = new Promise(function (resolve, reject) {
					backend.addtoCollection(_this2.getCardsByName(scrapedCollection, newCardNames), resolve, reject);
				});

				addtoPromise.then(function () {
